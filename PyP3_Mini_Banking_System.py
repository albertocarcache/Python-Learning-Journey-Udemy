...
def create_account(name: str, initial_balance: float):   
   ...
   for account in accounts:
      if account["name"].strip().lower() == name.strip().lower() :
         raise ValueError(f"Account with this name already exists: {account["name"]}")  
...
def deposit(name: str, amount: float):
   ...
   for account in accounts:
      if account["name"].strip().lower() == name.strip().lower():
         ...
         historial = account.get("transactions", [])
         historial.append({"type": "Deposit", "amount": amount})
         account["transactions"] = historial         

         #account.setdefault("transactions", []).append({"type": "Deposit", "amount": amount})                
...
def withdraw(name: str, amount: float):
   ...
   if amount <= 0:
      raise ValueError(f"Withdrawal amount must be greater than 0.\n       Account: {name} / Invalid Amount: ${amount}")
   ...
   for account in accounts:
      if account["name"].strip().lower() == name.strip().lower():      
         ...
         account.setdefault("transactions", []).append({"type": "Withdrawal", "amount": amount})
...
def show_account(name: str):
   ...
   for index, account in enumerate(accounts, start=1):
      if account["name"].strip().lower() == name.strip().lower(): 
         account_found = True # We mark that the account exists
         ...
         lista_trans = account.get("transactions", []) # We extract the list of dictionaries in the transactions field
