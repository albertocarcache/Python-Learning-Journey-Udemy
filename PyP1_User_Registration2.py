...
* When the first registration error is detected, the module displays the message "Registration failed".
  The error is stored in a dictionary of failed registrations.
"""
# ---------------------------------------------------------
...
registered_users = []
# A global list that compiles dictionaries of successful new user registrations.
# Contains names, emails, passwords, and status: "active".
failed_registrations = []
# A global list that compiles dictionaries of failed new user registrations.
...
def create_user_account(name: str, email: str, password: str):
    ...
    Prints:
        Registration successful & registration data: If registration succeeds, print
        Registration failed & the error message: If registration fails, print
     """
     format_ok, error_list = validate_user_data(name, email, password) # Unpack (True, errors)
     ...
     elif format_ok and is_duplicate:  
...
"""
Alternative negative logic to validate the inputs (format and email):
...
# 3) An invalid name
create_user_account("Jo", "jo@email.com", "Secure456")
# 4) An invalid email
